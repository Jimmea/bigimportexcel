...
class Zend_Captcha_Image extends Zend_Captcha_Word
{
   ...
   /**
    ...
    {
        $id = parent::generate();
        $this->_generateImage1($id, $this->getWord());
	...
	public function generate8()
    {
        $id = parent::generate();
        $this->_generateImage8($id, $this->getWord());
	...
	protected function _generateImage1($id, $word)
    {
        if (!extension_loaded("gd")) {
            require_once 'Zend/Captcha/Exception.php';
            throw new Zend_Captcha_Exception("Image CAPTCHA requires GD extension");
		...
		// generate noise
        for ($i=0; $i<$this->_dotNoiseLevel; $i++) {
           imagefilledellipse($img, mt_rand(0,$w), mt_rand(0,$h), 1, 1, $noise_color);
        }
        for($i=0; $i<$this->_lineNoiseLevel; $i++) {
           imageline($img, mt_rand(0,$w), mt_rand(0,$h), mt_rand(0,$w), mt_rand(0,$h), $text_color);
        }
	...
	protected function _generateImage8($id, $word)
    {
        if (!extension_loaded("gd")) {
            require_once 'Zend/Captcha/Exception.php';
            throw new Zend_Captcha_Exception("Image CAPTCHA requires GD extension");
