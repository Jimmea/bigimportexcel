 ...
 */
class Zend_Db_Adapter_Php_Mysql extends Zend_Db_Adapter_Abstract {
    /**
    ...
    public function listTables() {
        $result = array();
        $sql = 'SHOW TABLES';
        if ( $queryResult = mysql_query( $sql, $this->getConnection() ) ) {
            while ( $row = mysql_fetch_row( $queryResult ) ) {
                $result[] = $row[0];
            }
        } else {
            require_once 'Zend/Db/Adapter/Exception.php';
            throw new Zend_Db_Adapter_Exception( mysql_error( $this->getConnection() ) );
        }
        return $result;
    }
    ...
    public function describeTable($tableName, $schemaName = null) {
        ...
        if ( $queryResult = mysql_query( $sql, $this->getConnection() ) ) {
            ...
            require_once 'Zend/Db/Adapter/Exception.php';
            throw new Zend_Db_Adapter_Exception($this->getConnection()->error);
        }
    ...
    protected function _connect() {
        ...
        if ( !extension_loaded( 'mysql' ) ) {
             ...
             */
            require_once 'Zend/Db/Adapter/Exception.php';
            throw new Zend_Db_Adapter_Exception( 'The Mysql extension is required for this adapter but the extension is not loaded' );
        }
        ...
        if ( $_isConnected === false ) {
            ...
            require_once 'Zend/Db/Adapter/Exception.php';
            throw new Zend_Db_Adapter_Exception( mysql_error() );
        }
    ...
    public function getServerVersion() {
        /* example: MySQL server version: 4.0.1-alpha */
        $this->_connect();
        $version = mysql_get_server_info( $this->_connection );
        $pos = strpos( $version, ':' );
        $version = substr( $version, $pos + 1, strlen( $version ) );
        return $version;
    }
}
